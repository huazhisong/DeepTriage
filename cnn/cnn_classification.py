...
def get_embedding(input, pretrain_embedding_path,
                  embedding_type, vocabulary_size, emb_dim, vocab_file_path):

    init_width = 0.5 / emb_dim
    with tf.name_scope('embedding'):
        if embedding_type == 'rand':
            W = tf.Variable(tf.random_uniform(
                [vocabulary_size, emb_dim], -init_width, init_width), trainable=True, name="W")
        ...
        if embedding_type == 'multiple_channels':
            W = tf.Variable(tf.random_uniform(
                [vocabulary_size, emb_dim], -init_width, init_width), trainable=True, name="W")
            W.assign(initW)
            W_rand = tf.Variable(tf.random_uniform(
                [vocabulary_size, emb_dim], -init_width, init_width), trainable=True, name="W")
            embedded_rand = tf.nn.embedding_lookup(W_rand, input)
            embedded_chars = tf.nn.embedding_lookup(W, input)
            return tf.concat([tf.expand_dims(embedded_rand, -1),
                              tf.expand_dims(embedded_chars, -1)], 3)
...
def evaluation(logits, labels, top_k):
    ...
    with tf.name_scope("evaluation"):
        precision_op, precision = tf.contrib.metrics.streaming_sparse_precision_at_k(
            logits, label, top_k, name="precision")
        tf.summary.scalar("precision", precision)
        recall_op, recall = tf.contrib.metrics.streaming_sparse_recall_at_k(
            logits, label, top_k, name="recall")
